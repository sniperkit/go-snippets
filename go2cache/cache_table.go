...
// SetDataLoader configures a data-loader callback, which will be called when trying to access a non-existing key.
// The key and 0...n additional arguments are passed to the callback function.
...
func (table *CacheTable) expirationCheck() {
	...
	// To be more accurate with timers, we would need to update 'now' on every loop iteration.
	// Not sure it's really efficient though.
	now := time.Now()
...
// Unlike the Value method Exists neither tries to fetch data via the loadData callback nor does it
...
// NotFoundAdd tests whether an item not found in the cache.
// Unlike the Exists method this also adds data if they key could not be found.
