...
func TestCache(t *testing.T) {
	// add an expiring item after a non-expiring one to trigger expirationCheck iterating over non-expiring items
	table := Cache("testCache")
	table.Add(k+"_1", v, 0*time.Second)
	table.Add(k+"_2", v, 1*time.Second)
...
func TestCacheKeepAlive(t *testing.T) {
	...
	p := table.Add(k, v, 100*time.Millisecond)
...
func TestCallbacks(t *testing.T) {
	...
	i := table.Add(k, v, 500*time.Millisecond)
