...
func isKanjiLetter(ch rune) bool {
	return ch >= 0x80 && unicode.IsLetter(ch) && unicode.In(ch, unicode.Han)
}

func isNonkanjiLetter(ch rune) bool {
	return 'a' <= ch && ch <= 'z' || 'A' <= ch && ch <= 'Z' || ch == '_' || ch >= 0x80 && unicode.IsLetter(ch) && ! unicode.In(ch, unicode.Han)
}
...
func (s *Scanner) scanIdentifier() string {
	offs := s.offset
	for isNonkanjiLetter(s.ch) || isDigit(s.ch) {
		s.next()
	}
	return string(s.src[offs:s.offset])
}

func (s *Scanner) scanKanji() rune {
	offs := s.offset
	s.next()
	for _, n:= range string(s.src[offs:s.offset]) {
		return n
	}
