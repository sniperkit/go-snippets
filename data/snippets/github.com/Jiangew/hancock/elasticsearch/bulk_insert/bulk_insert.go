...
func main() {
	var(
		...
		typ = flag.String("type", "", "Elasticsearch index type name")
	...
	if *url == "" {
		log.Fatal("missing url param")
	}
	if *index == "" {
		log.Fatal("missing index param")
	}
	if *typ == "" {
		log.Fatal("missing type param")
	}
	if *n <= 0 {
	...
	// Set a group of goroutines from the excellent errgroup package
