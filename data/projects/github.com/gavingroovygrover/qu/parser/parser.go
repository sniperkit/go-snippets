...
func (p *parser) parseExprList(lhs bool) (list []ast.Expr) {
	...
	if p.kanjiMode && p.tok.IsKeyword() {
		...
		if scanner.Kanjis[r].Kind != scanner.KeywordKanji  {
			p.tok = token.IDENT
		}
	}
...
func (p *parser) parseBranchStmt(tok token.Token) *ast.BranchStmt {
	...
	oldKanjiMode:= p.kanjiMode
	if ( p.lit == "跳" || p.lit == "破" || p.lit == "继" ) && ! oldKanjiMode {
		p.kanjiMode = true
	}
	pos := p.expect(tok)
	var label *ast.Ident
...
func (p *parser) parseCaseClause(typeSwitch bool) *ast.CaseClause {
	...
	oldKanjiMode:= p.kanjiMode
	if ( p.lit == "事" || p.lit == "别" ) && ! oldKanjiMode {
		p.kanjiMode = true
	}
	pos := p.pos
	var list []ast.Expr
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseGenDecl(keyword token.Token, f parseSpecFunction, oldKanjiMode bool) *ast.GenDecl {
	if p.trace {
...
func (p *parser) parseDecl(sync func(*parser)) ast.Decl {
	...
	case token.CONST, token.VAR:
		if ( p.lit == "久" || p.lit == "变" ) && ! oldKanjiMode {
			p.kanjiMode = true
		}
		f = p.parseValueSpec

	case token.TYPE:
		if p.lit == "种" && ! oldKanjiMode {
			p.kanjiMode = true
		}
		f = p.parseTypeSpec
	...
	return p.parseGenDecl(p.tok, f, oldKanjiMode)
}
...
func (p *parser) parseFile() *ast.File {
	...
	/*for _, ki := range p.kanjiImports {
		p.imports = append(p.imports, ki)
		decls = append(decls, &ast.GenDecl{ //TODO: make this work
			TokPos: p.pos,
			Tok:    token.IMPORT,
			Lparen: p.pos,
			Specs:  []ast.Spec{ki},
