package main

import (
	"fmt"
	"net/http"
	"log"
	"net"
	"io"
	consulapi "github.com/hashicorp/consul/api"
	"github.com/go-redis/redis"
	reflectInvoke "github.com/jiangew/hancock/utils/reflect_invoke"
	"github.com/jiangew/hancock/ranklist/cache"
	"github.com/jiangew/hancock/ranklist/service"
)

const RECV_BUF_LEN = 1024

func consulCheck(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintln(w, "consulCheck")
}

func registerServer() {
	config := consulapi.DefaultConfig()
	client, err := consulapi.NewClient(config)
	if err != nil {
		log.Fatal("consul client error : ", err)
	}

	checkPort := 8081

	registration := new(consulapi.AgentServiceRegistration)
	registration.ID = "rankNode_1"
	registration.Name = "rankNode"
	registration.Port = 9528
	registration.Tags = []string{"serverNode"}
	registration.Address = "127.0.0.1"
	registration.Check = &consulapi.AgentServiceCheck{
		HTTP:                           fmt.Sprintf("http://%s:%d%s", registration.Address, checkPort, "/check"),
		Timeout:                        "3s",
		Interval:                       "5s",
		DeregisterCriticalServiceAfter: "30s", //check失败后30秒删除本服务
	}

	err = client.Agent().ServiceRegister(registration)
	if err != nil {
		log.Fatal("register server error : ", err)
	}

	http.HandleFunc("/check", consulCheck)
	http.ListenAndServe(fmt.Sprintf(":%d", checkPort), nil)
}

func RankServer(conn net.Conn) {
	buf := make([]byte, RECV_BUF_LEN)
	defer conn.Close()

	for {
		n, err := conn.Read(buf)
		switch err {
		case nil:
			log.Println("get:", string(buf[0:n]))

			resultData := reflectInvoke.InvokeByJson([]byte(buf[0:n]))
			conn.Write(resultData)

		case io.EOF:
			log.Printf("Warning: End of data: %s\n", err)
			return

		default:
			log.Printf("Error: Reading data: %s\n", err)
			return
		}
	}
}

func main() {
	rdsClient := redis.NewClient(&redis.Options{
		Addr:     fmt.Sprintf("%s:%d", "127.0.0.1", 6379),
		Password: "123456",
		DB:       0,
	})

	cache.Rds = rdsClient
	service.Rds = rdsClient

	for i := 0; i < 5; i++ {
		player := &cache.PlayerInfo{
			PlayerId:   i + 1,
			PlayerName: fmt.Sprintf("玩家 %d", i+1),
			Level:      1,
			Online:     true,
		}

		cache.SetPlayerInfo(player)
	}

	// 用json字符串调用
	reflectInvoke.RegisterMethod(service.DefaultRankService)

	// 服务注册
	go registerServer()

	ln, err := net.Listen("tcp", "0.0.0.0:9528")
	if nil != err {
		panic("Error: " + err.Error())
	}

	for {
		conn, err := ln.Accept()
		if nil != err {
			panic("Error: " + err.Error())
		}

		go RankServer(conn)
	}
}
